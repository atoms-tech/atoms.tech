...
function generateTimestamps(daysBack: number, activitiesPerDay: number): Date[] {
    ...
    for (let day = 0; day < daysBack; day++) {
        ...
        const activitiesForDay = Math.floor(activitiesPerDay * (0.5 + Math.random()));
...
function selectRandomActivity(): string {
    const totalWeight = ACTIVITY_TYPES.reduce((sum, type) => sum + type.weight, 0);
...
function generateActivityData(entityType: string, action: string, entityData: any) {
    const baseData = {
        id: entityData.id,
    ...
    switch (action) {
        ...
        case 'duplicated':
            return {
                old_data: baseData,
                new_data: { ...baseData, id: `${baseData.id}_copy`, name: `${baseData.name} (copy)` }
            };
...
async function populateAnalyticsData(options: PopulateOptions) {
    ...
    try {
        ...
        const users = orgMembers?.map(m => m.profiles).filter(Boolean) || [];
        
        if (documents?.length === 0 || users.length === 0) {
            console.log('❌ No documents or users found. Cannot populate analytics data.');
            return;
        }
        
        console.log(`📊 Found ${documents?.length} documents, ${blocks?.length} blocks, ${users.length} users`);
        
        // Generate timestamps
        const timestamps = generateTimestamps(daysBack, activitiesPerDay);
        console.log(`⏰ Generated ${timestamps.length} activity timestamps over ${daysBack} days`);
        ...
        for (const timestamp of timestamps) {
            ...
            const { old_data, new_data } = generateActivityData(entity.type, action, entity);
            ...
            auditLogs.push({
                ...
                metadata: {
                    version: (entity.version || 1) + Math.floor(Math.random() * 5),
        ...
        for (let i = 0; i < auditLogs.length; i += batchSize) {
            ...
            console.log(`✅ Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(auditLogs.length / batchSize)} (${inserted}/${auditLogs.length} total)`);
        }
...
async function main() {
    ...
    if (!orgId) {
        console.error('❌ Usage: tsx scripts/populate-analytics-data.ts <orgId> [projectId]');
        console.error('   Example: tsx scripts/populate-analytics-data.ts 46bddba6-f612-4bb8-b5d0-5b0be00c945c');
        process.exit(1);
    }
