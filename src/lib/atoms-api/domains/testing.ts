...
export function createTestingDomain(supabase: SupabaseAny) {
  return {
    async listProjectTests(
      projectId: string,
      filters: TestFilters = {},
      pagination: Pagination = { page: 1, pageSize: 10, orderBy: 'created_at', orderDirection: 'desc' },
      ...
      try {
        const { data, error, count } = await query as unknown as { data: Tables<'test_req'>[]; error: unknown; count: number };
    ...
    async listRelationsByTest(testId: string): Promise<Tables<'requirement_tests'>[]> {
      try {
        const { data, error } = await supabase
          .from('requirement_tests')
          .select('*')
    ...
    async listRelationsByRequirement(requirementId: string): Promise<Tables<'requirement_tests'>[]> {
      try {
        const { data, error } = await supabase
          .from('requirement_tests')
          .select('*')
    ...
    async listRelationsByTests(testIds: string[]): Promise<Tables<'requirement_tests'>[]> {
    ...
    async createTest(testData: TablesInsert<'test_req'>): Promise<Tables<'test_req'>> {
      try {
        const { data, error } = await supabase
          .from('test_req')
          .insert(testData)
    ...
    async updateTest(id: string, updates: TablesUpdate<'test_req'>): Promise<Tables<'test_req'>> {
      try {
        const { data, error } = await supabase
          .from('test_req')
          .update(updates)
    ...
    async createRelation(relationData: TablesInsert<'requirement_tests'>): Promise<Tables<'requirement_tests'>> {
      try {
        const { data, error } = await supabase
          .from('requirement_tests')
          .insert(relationData)
    ...
    async updateRelation(requirementId: string, testId: string, updates: TablesUpdate<'requirement_tests'>): Promise<Tables<'requirement_tests'>> {
      try {
        const { data, error } = await supabase
          .from('requirement_tests')
          .update(updates)
