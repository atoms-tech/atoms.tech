...
export function createAuthDomain(supabase: SupabaseAny) {
    return {
        ...
        async listProfiles(
            select: string = 'email, full_name, id, is_approved, created_at',
        ) {
            ...
            return (
                (data as unknown as Array<
                    Pick<
                        Profile,
                        'email' | 'full_name' | 'id' | 'is_approved' | 'created_at'
                    >
                >) ?? []
            );
        },
        ...
        async signInWithPassword(
            ...
            return await (supabase as SupabaseServerClient).auth.signInWithPassword(
                credentials,
            );
        },
        ...
        async signUp(
            ...
            return await (supabase as SupabaseServerClient).auth.signUp(
        ...
        async exchangeCodeForSession(code: string): Promise<AuthResponse> {
            return await (supabase as SupabaseServerClient).auth.exchangeCodeForSession(
                code,
            );
        },
