...
export function createPropertiesDomain(supabase: SupabaseAny) {
    return {
        ...
        async createMany(
            props: import('@/types/base/database.types').TablesInsert<'properties'>[],
        ): Promise<Property[]> {
            try {
                const { data, error } = await supabase
                    .from('properties')
        ...
        async softDelete(id: string, _userId: string): Promise<void> {
            try {
                // properties table does not have soft-delete fields; perform hard delete
        ...
        async updatePositions(
            updates: { id: string; position: number; updated_by: string }[],
        ): Promise<void> {
            try {
                await Promise.all(
                    updates.map((u) =>
