...
export function createDiagramsDomain(supabase: SupabaseAny) {
  return {
    async listByProject(projectId: string): Promise<Pick<Diagram, 'id'|'name'|'thumbnail_url'|'updated_at'|'created_by'>[]> {
      try {
        ...
        return (data as unknown as Pick<Diagram, 'id'|'name'|'thumbnail_url'|'updated_at'|'created_by'>[]) ?? [];
      } catch (e) {
