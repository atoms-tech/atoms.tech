...
export function createRealtimeDomain(supabase: SupabaseAny) {
  return {
    subscribeBlocks(documentId: string, handlers: {
      onInsert?: (b: Block) => void,
      onUpdate?: (b: Block) => void,
      onDelete?: (oldId: string) => void,
    }) {
      const channelFactory = (supabase as unknown as { channel: (name: string) => RealtimeChannel }).channel;
      const channel = channelFactory(`blocks:${documentId}`)
        .on('postgres_changes', {
          event: 'INSERT', schema: 'public', table: 'blocks', filter: `document_id=eq.${documentId}`,
        }, (payload: unknown) => {
          const p = payload as RealtimePostgresChangesPayload<Block>;
          handlers.onInsert?.(p.new);
        })
        .on('postgres_changes', {
          event: 'UPDATE', schema: 'public', table: 'blocks', filter: `document_id=eq.${documentId}`,
        }, (payload: unknown) => {
          const p = payload as RealtimePostgresChangesPayload<Block>;
          handlers.onUpdate?.(p.new);
        })
        .on('postgres_changes', {
          event: 'DELETE', schema: 'public', table: 'blocks', filter: `document_id=eq.${documentId}`,
        }, (payload: unknown) => {
          const p = payload as RealtimePostgresChangesPayload<Block>;
    ...
    subscribeRequirements(documentId: string, handlers: {
      ...
      const channel = channelFactory(`requirements:${documentId}`)
        .on('postgres_changes', {
          event: 'INSERT', schema: 'public', table: 'requirements', filter: `document_id=eq.${documentId}`,
        }, (payload: unknown) => {
          const p = payload as RealtimePostgresChangesPayload<Requirement>;
          handlers.onInsert?.(p.new);
        })
        .on('postgres_changes', {
          event: 'UPDATE', schema: 'public', table: 'requirements', filter: `document_id=eq.${documentId}`,
        }, (payload: unknown) => {
          const p = payload as RealtimePostgresChangesPayload<Requirement>;
          handlers.onUpdate?.(p.new);
        })
        .on('postgres_changes', {
          event: 'DELETE', schema: 'public', table: 'requirements', filter: `document_id=eq.${documentId}`,
        }, (payload: unknown) => {
          const p = payload as RealtimePostgresChangesPayload<Requirement>;
    ...
    subscribeColumns(documentId: string, onAnyChange: () => void) {
      const channelFactory = (supabase as unknown as { channel: (name: string) => RealtimeChannel }).channel;
      const channel = channelFactory(`columns:${documentId}`)
        .on('postgres_changes', {
