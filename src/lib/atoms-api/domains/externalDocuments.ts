  ...
  return {
    ...
    async upload(file: File, orgId: string): Promise<ExternalDocument> {
      ...
      if (typeof window !== 'undefined') {
        ...
        const res = await fetch('/api/external-documents/upload', { method: 'POST', body: form });
        if (!res.ok) {
          const j = await res.json().catch(() => ({}));
          throw normalizeError(new Error(j.error || 'Upload failed'), 'Failed to upload external document');
        }
      ...
      const { data: document, error: documentError } = await supabase
        .from('external_documents')
        .insert({ name: file.name, type: file.type, organization_id: orgId, size: (file as unknown as { size?: number }).size ?? null })
        .select('*')
        .single();
      if (documentError) throw normalizeError(documentError, 'Failed to create external document record');
    ...
    async remove(documentId: string): Promise<void> {
      if (typeof window !== 'undefined') {
        const res = await fetch(`/api/external-documents/delete?id=${encodeURIComponent(documentId)}`, { method: 'DELETE' });
        if (!res.ok) {
          const j = await res.json().catch(() => ({}));
          throw normalizeError(new Error(j.error || 'Delete failed'), 'Failed to delete external document');
        }
        return;
      }
      ...
      const { data: document, error: fetchError } = await supabase
        ...
        .eq('id', documentId)
        .single();
      if (fetchError) throw normalizeError(fetchError, 'Failed to fetch external document');
      ...
      const { error: storageError } = await supabase.storage
        .from('external_documents')
        .remove([filePath]);
      if (storageError) throw normalizeError(storageError, 'Failed to remove external document from storage');
    ...
    async update(documentId: string, data: Partial<ExternalDocument>): Promise<ExternalDocument> {
