...
export function createDocumentsDomain(supabase: SupabaseAny) {
  return {
    ...
    async listColumnsByBlockIds(blockIds: string[]): Promise<(ColumnRow & { property: Tables<'properties'> | null })[]> {
      ...
      if (error) throw error;
      return (data as unknown as Array<ColumnRow & { property: Tables<'properties'> | null }>) ?? [];
    },
