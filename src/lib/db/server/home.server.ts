 ...
 */
export const getUserProjectsAcrossOrgsServer = async (userId: string): Promise<ProjectWithOrg[]> => {
 ...
 */
export const getUserRecentActivityServer = async (userId: string, limit: number = 15): Promise<RecentActivity[]> => {
    const supabase = await createClient();
        ...
        try {
            ...
            if (log.entity_type === 'project') {
                ...
                if (document) {
                    ...
                    organizationName = document.projects?.organizations?.name || '';
                }
            } else if (log.entity_type === 'requirement') {
                const { data: requirement } = await supabase
                    .from('requirements')
                    .select('name, documents(name, projects(name, organizations(name)))')
                    .eq('id', log.entity_id)
                    .single();
                ...
                if (requirement) {
                    ...
                    organizationName = requirement.documents?.projects?.organizations?.name || '';
                }
            }
        } catch (error) {
...
export const getUserRecentActivityPaginatedServer = async (
    ...
    return {
        ...
        hasMore,
        nextCursor: hasMore ? activities[activities.length - 1].created_at : undefined,
        total: total || 0
    };
};
 ...
 */
export const getUserOnboardingProgressServer = async (userId: string): Promise<OnboardingProgress> => {
    const supabase = await createClient();
