...
export function useSlashCommands(options: UseSlashCommandsOptions = {}): UseSlashCommandsReturn {
    ...
    /**
     * Get cursor position on screen for dropdown positioning
     */
    const getCursorScreenPosition = useCallback((): { x: number; y: number } | null => {
        if (editor) {
            // For TipTap, we'll use the editor's DOM position
    ...
    }, [detectSlashCommand, searchCommands, editor, inputRef, getCursorScreenPosition]);
