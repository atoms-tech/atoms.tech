...
export function useClipboard(options: UseClipboardOptions = {}) {
    ...
    const copyHTML = useCallback((html: string, fallbackText?: string) => {
        return copyToClipboard({ html, text: fallbackText || html });
    }, [copyToClipboard]);

    const copyJSON = useCallback((json: any, fallbackText?: string) => {
    ...
    const copyTableData = useCallback((data: any[][], headers?: string[]) => {
        const text = [
