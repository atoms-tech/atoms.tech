    ...
    return useMutation<RestoreVersionResult[], Error, RestoreVersionInput[]>({
        mutationFn: async (inputs: RestoreVersionInput[]) => {
            ...
            for (const input of inputs) {
                try {
                    ...
                    console.error(`Failed to restore version for ${input.entityId}:`, error);
                    throw error;
                }
            }
            return results;
        ...
        onError: (error, variables) => {
            ...
            toast.error('Failed to restore versions', {
                description: error.message || 'Some restorations may have failed'
            });
        },
    });
}
...
export function useCreateRestorePoint() {
    ...
    return useMutation<{ success: boolean; backupId: string }, Error, {
        entityId: string;
        entityType: 'document' | 'block' | 'requirement';
        reason?: string;
    }>({
        mutationFn: async ({ entityId, entityType, reason }) => {
        ...
        onSuccess: (result, variables) => {
            ...
            queryClient.invalidateQueries({
                queryKey: analyticsQueryKeys.versionHistory(variables.entityId, variables.entityType)
            });
        },
        onError: (error) => {
