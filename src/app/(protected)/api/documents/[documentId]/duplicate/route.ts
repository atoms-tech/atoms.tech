...
export async function POST(
    ...
    try {
        ...
        const canCreateDocuments = ['owner', 'admin', 'maintainer', 'editor'].includes(userRole);
        ...
        const finalSlug = existingDoc ? `${duplicatedSlug}-${Date.now()}` : duplicatedSlug;
        ...
        if (docError) {
            return NextResponse.json(
                { error: 'Failed to create document copy', details: docError.message },
                { status: 500 }
            );
        }
        ...
        if (includeRequirements) {
            ...
            if (blocksError) {
                ...
                await supabase
                    .from('documents')
                    .update({ is_deleted: true, deleted_by: user.id, deleted_at: timestamp })
                    .eq('id', newDocumentId);

                return NextResponse.json(
                    { error: 'Failed to fetch source blocks', details: blocksError.message },
                    { status: 500 }
                );
            }
            ...
            if (sourceBlocks && sourceBlocks.length > 0) {
                ...
                if (createBlocksError) {
                    ...
                    return NextResponse.json(
                        { error: 'Failed to create blocks', details: createBlocksError.message },
                        { status: 500 }
                    );
                }
                ...
                for (let i = 0; i < sourceBlocks.length; i++) {
                    ...
                    if (sourceBlock.requirements && sourceBlock.requirements.length > 0) {
                        const newRequirements = sourceBlock.requirements
                            ...
                            .map((req: any) => ({
                                ...
                                enchanced_requirement: req.enchanced_requirement,
                ...
                if (allRequirements.length > 0) {
                    ...
                    if (requirementsError) {
                        console.error('Failed to copy requirements:', requirementsError);
        ...
        if (includeProperties) {
            ...
            if (!propsError && sourceProperties && sourceProperties.length > 0) {
