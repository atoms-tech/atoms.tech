...
export function AnalyticsWidget({ instance, data }: WidgetProps) {
    ...
    const enterpriseOrgs = organizations.filter((org: any) => org.type === 'enterprise').length;
    const teamOrgs = organizations.filter((org: any) => org.type === 'team').length;
        ...
        <Card className="h-full bg-gradient-to-br from-gray-900/50 to-gray-800/50 border-gray-700">
            ...
            <CardContent className="h-full overflow-auto space-y-4">
                ...
                <div className={`grid gap-3 ${
                    instance.size.width > 500 ? 'grid-cols-3' : 
                    instance.size.width > 350 ? 'grid-cols-2' : 'grid-cols-1'
                }`}>
                    {metrics.slice(0, displayMode === 'compact' ? 3 : metrics.length).map((metric, index) => (
                        <motion.div
                            key={metric.label}
                    ...
                    <div className="bg-black/30 rounded-lg p-4 border border-gray-700">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-center">
                            <div>
                                ...
                                <div className="text-xs text-gray-400">
                                    {Math.floor(weeklyGoalProgress / 10)} actions completed
                                </div>
                            </div>
                            <div>
                                <div className="text-sm font-medium text-white">This Month</div>
                                <div className="text-xs text-gray-400">
                                    {Math.floor(daysActive * 2.5)} total activities
                                </div>
                            </div>
                        </div>
                    </div>
