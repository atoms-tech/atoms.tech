...
import { generateReqIdFromDocument } from '@/lib/utils/reqIdGenerator';
    ...
    const saveRequirement = async (
        ...
        try {
            ...
            if (isNew) {
                ...
                // Generate unique REQ-ID for this requirement
                const external_id = naturalFields?.external_id || await generateReqIdFromDocument(documentId);
                console.log('Generated REQ-ID for new requirement:', external_id);

                const newRequirementData = {
                    ...requirementData,
                    created_by: userId,
                    name: naturalFields?.name || 'New Requirement', // Default name for new requirements
                    external_id, // Use generated or provided REQ-ID
