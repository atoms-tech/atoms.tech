...
export const LazyRecentActivityWidget = dynamic(
    () => import('./RecentActivityWidget').then(mod => ({ default: mod.RecentActivityWidget })),
    {
        loading: () => (
...
export const LazyProjectSelectionGrid = dynamic(
    () => import('./ProjectSelectionGrid').then(mod => ({ default: mod.ProjectSelectionGrid })),
    {
        loading: () => (
...
export const LazyVirtualizedProjectGrid = dynamic(
    () => import('./VirtualizedProjectGrid').then(mod => ({ default: mod.VirtualizedProjectGrid })),
    {
        loading: () => (
            <div className="space-y-4">
                <div className="h-8 w-32 bg-muted animate-pulse rounded" />
...
export const LazyOnboardingWidget = dynamic(
    () => import('./OnboardingWidget').then(mod => ({ default: mod.OnboardingWidget })),
    {
        loading: () => (
...
// Bundle analyzer helper - only loads in development
