        ...
        if ('PerformanceObserver' in window) {
            observerRef.current = new PerformanceObserver((list) => {
                ...
                entries.forEach((entry) => {
                    // Only monitor resource timing entries
                    ...
                    if (
                        ...
                        resourceEntry.initiatorType === 'image'
                    ) {
    ...
    const analytics = {
        ...
        averageLoadTime:
            performanceData.reduce(
                (sum, item) => sum + item.metrics.loadTime,
                0,
            ) / performanceData.length || 0,
        cacheHitRate:
            (performanceData.filter((item) => item.metrics.fromCache).length /
                performanceData.length) *
                100 || 0,
        formatDistribution: performanceData.reduce(
            (acc, item) => {
        ...
        slowImages: performanceData.filter(
            (item) => item.metrics.loadTime > 1000,
        ), // Images taking more than 1s
    };
    ...
    return (
        <div className="fixed bottom-4 right-4 bg-black/80 text-white p-4 rounded-lg text-sm max-w-sm z-50">
            ...
            <div className="space-y-1">
                ...
                <div>
                    Avg Load Time: {analytics.averageLoadTime.toFixed(0)}ms
                </div>
                ...
                <div className="mt-2">
                    ...
                    {Object.entries(analytics.formatDistribution).map(
                        ([format, count]) => (
...
export function useImagePerformance() {
    const [metrics, setMetrics] = useState<
        (ImagePerformanceMetrics & { url: string })[]
    >([]);
    ...
    return {
        ...
        averageLoadTime:
            ...
            0,
        cacheHitRate:
            (metrics.filter((m) => m.fromCache).length / metrics.length) *
                100 || 0,
    };
}
