    ...
    useEffect(() => {
        ...
        const checkUser = async () => {
            try {
                ...
                // In development, if session check times out, assume not authenticated
                // This handles the case where Supabase backend routes to atoms.tech instead of localhost
                if (
                    error instanceof Error &&
                    error.message === 'Session check timeout'
                ) {
                    console.log(
                        'useAuth: Session check timed out - assuming not authenticated in development',
                    );
                }
